...
const SUPABASE_URL = 'https://zvdtsedtzscvukzqkwhy.supabase.co'; 
const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Inp2ZHRzZWR0enNjdnVrenFrd2h5Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTM0NDgyMTQsImV4cCI6MjA2OTAyNDIxNH0.m75Z4D-_TfUcc5iatxcL8gmBEv8pM4-A2JU779gdx-w';
...
class MediSecureApp {
    ...
    handleConnectionError(error) {
        ...
        if (this.connectionRetryCount <= this.maxRetries) {
            showConnectionStatus(`Connection failed. Retrying... (${this.connectionRetryCount}/${this.maxRetries})`, 'error', 3000);
            setTimeout(() => this.testDatabaseConnection(), 3000);
        } else {
            showConnectionStatus('Unable to connect to database. Please refresh the page.', 'error', 0);
    ...
    showOfflineMode() {
        ...
        this.showNotification('Application is running in offline mode. Some features may not work.', 'error');
    ...
    async checkExistingSession() {
        const sessionData = localStorage.getItem('hospitalSession') || sessionStorage.getItem('hospitalSession');
        if (sessionData) {
            try {
    ...
    initializeEventListeners() {
        ...
        if (togglePassword) togglePassword.addEventListener('click', () => this.togglePasswordVisibility('password', 'togglePassword'));
        if (passwordInput) passwordInput.addEventListener('input', () => this.checkPasswordStrength());
        if (confirmPasswordInput) confirmPasswordInput.addEventListener('input', () => this.validatePasswordMatch());
        ...
        if (toggleLoginPassword) toggleLoginPassword.addEventListener('click', () => this.togglePasswordVisibility('loginPassword', 'toggleLoginPassword'));
        ...
        if (verifyPatientBtn) verifyPatientBtn.addEventListener('click', () => this.verifyPatient());
        if (addRecordForm) addRecordForm.addEventListener('submit', (e) => this.handleAddRecord(e));
        if (logoutBtn) logoutBtn.addEventListener('click', () => this.handleLogout());
        if (attachmentsInput) attachmentsInput.addEventListener('change', (e) => this.handleFileSelection(e));
        ...
        if (patientIdInput) patientIdInput.addEventListener('input', () => this.autoVerifyPatient());
        if (patientPhoneInput) patientPhoneInput.addEventListener('input', () => this.autoVerifyPatient());
        if (patientNameInput) patientNameInput.addEventListener('input', () => this.autoVerifyPatient());
    ...
    async showDashboard() {
        ...
        if (this.currentSession) {
            document.getElementById('dashboardHospitalName').textContent = this.currentSession.hospitalName;
            document.getElementById('dashboardHospitalId').textContent = `ID: ${this.currentSession.hospitalId}`;
    ...
    switchDashboardView(view) {
        ...
        const views = ['dashboardView', 'recordsView', 'patientsView', 'analyticsView', 'settingsView'];
        ...
        const activeMenuItem = document.querySelector(`[onclick="app.switchDashboardView('${view}')"]`)?.closest('.menu-item');
        ...
        recordsList.innerHTML = records.map(record => `
            <div class="record-item" style="background: white; padding: 1.5rem; border-radius: 12px; margin-bottom: 1rem; box-shadow: 0 3px 6px rgba(0,0,0,0.1); border: 1px solid #e2e8f0;">
                <div class="record-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem;">
                    <h4 style="margin: 0; color: #2d3748; font-size: 1.2rem;">${record.patients.name}</h4>
                    <span class="record-type" style="background: #667eea; color: white; padding: 0.25rem 0.75rem; border-radius: 20px; font-size: 0.8rem; text-transform: capitalize;">${record.record_type.replace('_', ' ')}</span>
                </div>
                <div class="record-details" style="margin-bottom: 1rem;">
                    <p style="margin: 0.25rem 0;"><strong>Diagnosis:</strong> ${record.diagnosis}</p>
                    <p style="margin: 0.25rem 0;"><strong>Doctor:</strong> ${record.doctor_name}</p>
                    <p style="margin: 0.25rem 0;"><strong>Date:</strong> ${new Date(record.created_at).toLocaleDateString()}</p>
                    <p style="margin: 0.25rem 0;"><strong>Severity:</strong> <span style="color: ${this.getSeverityColor(record.severity)}">${record.severity}</span></p>
                </div>
                <div class="record-actions">
                    <button class="btn secondary" onclick="app.viewRecord('${record.id}')" style="padding: 0.5rem 1rem; border: 1px solid #e2e8f0; background: #f7fafc; border-radius: 8px; cursor: pointer;">
    ...
    resetForms() {
        ...
        const detailSections = document.querySelectorAll('.hospital-card, .patient-verified-card');
    ...
    checkDatabaseAvailability() {
        if (!isSupabaseInitialized) {
            this.showNotification('Database connection not available. Please refresh the page.', 'error');
            return false;
        }
        return true;
    }
    ...
    async verifyHospital() {
        ...
        try {
            ...
            if (error || !data) {
                this.showStatus(statusDiv, 'Hospital ID not found. Please check and try again.', 'error');
                return;
            }

            if (!data.is_verified) {
                this.showStatus(statusDiv, 'Hospital is not verified. Please contact administration.', 'error');
                return;
            }
            ...
            this.showStatus(statusDiv, 'Error verifying hospital. Please try again.', 'error');
        } finally {
    ...
    displayHospitalDetails(hospital) {
        ...
        document.getElementById('hospitalLocation').textContent = `${hospital.city}, ${hospital.state}`;
        document.getElementById('hospitalType').textContent = hospital.hospital_type;
        document.getElementById('hospitalLicense').textContent = hospital.license_number;
    }
    ...
    checkPasswordStrength() {
        ...
        strengthText.className = `strength-text ${strength >= 3 ? 'success' : 'error'}`;
    }
    ...
    async handleSignup(e) {
        ...
        try {
            ...
            this.showNotification('Hospital account created successfully!', 'success');
    ...
    async handleLogin(e) {
        ...
        try {
            ...
            if (error || !data) {
                throw new Error('Invalid credentials. Please check your Hospital ID and password.');
            }

            if (!data.is_verified) {
                throw new Error('Hospital account is not verified. Please contact administration.');
            }
    ...
    handleForgotPassword(e) {
        ...
        if (hospitalId) {
            this.showNotification(`Password reset instructions will be sent to the registered email for Hospital ID: ${hospitalId.toUpperCase()}`, 'info');
        }
    }
    ...
    async verifyPatient() {
        ...
        if (!patientId || !patientPhone || !patientName) {
            this.showNotification('Please fill in Patient ID, Phone, and Name', 'error');
            return;
        }
        ...
        try {
            ...
            if (patientError || !patientData) {
                this.showStatus(patientIdStatus, 'Patient ID not found in our records', 'error');
                return;
            }

            // Verify phone number matches
            if (patientData.phone !== patientPhone) {
                this.showStatus(patientPhoneStatus, 'Phone number does not match our records', 'error');
                return;
            }

            // Verify name matches (case insensitive)
            if (patientData.name.toLowerCase() !== patientName.toLowerCase()) {
                this.showStatus(patientNameStatus, 'Name does not match our records', 'error');
                return;
            }

            // All verification passed
            this.verifiedPatient = patientData;
            this.showStatus(patientIdStatus, 'Patient verified successfully!', 'success');
            ...
            this.showNotification('Error verifying patient. Please try again.', 'error');
        } finally {
            verifyBtn.disabled = false;
        }
    }
    ...
    autoVerifyPatient() {
        ...
        if (patientId.length >= 6 && patientPhone.length >= 10 && patientName.length >= 3) {
    ...
    displayVerifiedPatient(patient) {
        ...
        document.getElementById('verifiedPatientEmail').textContent = patient.email || 'N/A';
        document.getElementById('verifiedPatientAge').textContent = age + (age !== 'N/A' ? ' years' : '');
        document.getElementById('verifiedPatientGender').textContent = patient.gender || 'N/A';
        document.getElementById('verifiedPatientBloodGroup').textContent = patient.blood_group || 'N/A';
    ...
    handleDatabaseError(error) {
        ...
        if (error.message) {
            if (error.message.includes('duplicate key')) {
                ...
                errorMessage = 'Invalid patient or hospital reference. Please verify patient again.';
